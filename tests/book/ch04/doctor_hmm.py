...
states = ('Healthy', 'Fever')#元组
start_probability = {'Healthy': 0.6, 'Fever': 0.4}#Dictionary字典，就类似java中的Map集合，以“键值对”的形式存放数据，key不能重复
...
for O, S in given_model.generate(3, 5, 2):#样本生成算法,由于随机数的原因，每次运行结果都是随机的，但一定满足给定隐马尔可夫模型的约束。
    print(" ".join((observations_index_label[o] + '/' + states_index_label[s]) for o, s in zip(O, S)))

trained_model = FirstOrderHiddenMarkovModel()
trained_model.train(given_model.generate(3, 10, 100000))#3:序列最低长度、10序列最高长度，100000样本生成数
